//
//  DTPivotDashboardViewController.m
//  FN3
//
...
#import "DTPivotDashboardViewController.h"
...
@implementation DTPivotDashboardViewController
...
- (void)viewDidLoad
{
    ...
    self.directionView.isLateral = NO;
    ...
    self.speedDepthView.customInputAccessoryView = [self inputNavbarView:NSLocalizedString(@"Pivot speed %", nil)];
...
{    
    ...
    for (DTField *field in editableFields) {
        ...
        if([field isKindOfClass:[DTToggleField class]])
        {
            DTToggleField *toggleField = (DTToggleField *) field;
            NSLog(@"%@", toggleField.name);
        }
...
- (void)initializeFromEquipment
{
    ...
    pivot = (DTPivot *)[DTEquipment equipmentWithId:self.equipmentId 
                                          inContext:store.managedObjectContext];
    
    [pivotView configureFromEquipment:pivot];
    pivotView.detailLevel = DTEquipmentDetailLevelDetail;
    pivotView.borderWidth = 7;
    ...
    [(DTConnectionView *)[self.headerView viewWithTag:1] setCommStatus:pivot.commStatus];
    
    if (!self.pollButton.isInPollError) {
        NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
        formatter.dateStyle = formatter.timeStyle = NSDateFormatterShortStyle;
        [(UILabel *)[self.headerView viewWithTag:2] setText:[formatter stringFromDate:pivot.lastUpdated]];
        [(UILabel *)[self.headerView viewWithTag:2] setTextColor:[UIColor whiteColor]];
    }
    
    [self.planView setDriver:pivot.driver planId:pivot.planId step:pivot.planStepValue];
    
    [(UILabel *)[self.statusSummaryView viewWithTag:1] setText:pivot.subtitle];
    if (pivot.position) {
        [(UILabel *)[self.currentPositionView viewWithTag:1] setText:[NSString stringWithFormat:@"%@%@", pivot.position, pivot.positionUom]];
    } else {
        [(UILabel *)[self.currentPositionView viewWithTag:1] setText:nil];
    }
    
    [(UILabel *)[self.durationView viewWithTag:1] setText:pivot.durationDescription];
    
    [self.directionView setDirection:pivot.directionOption];
    
    NSString *depthTitle = [NSString stringWithFormat:@"%@ (%@)", NSLocalizedString(@"Depth to apply", nil), pivot.depthUom];
    self.speedDepthView.speedTitle = NSLocalizedString(@"Speed %", nil);
    self.speedDepthView.depthTitle = depthTitle;
    [self.speedDepthView.depthField setValue:0 units:[NSString stringWithFormat:@" %@", pivot.depthUom]];
    
    //self.speedDepthView.depthConversionFactor = pivot.depthConversionFactor;
    [self.speedDepthView setDepthConversionFactor:pivot.depthConversionFactor];
    [self.speedDepthView.speedField setValue:pivot.rate units:@"%"];
    [self.speedDepthView setSpeed:pivot.rate];
    self.speedDepthView.isWaterOn = [pivot.water boolValue];
    
    self.waterView.on = [pivot.water boolValue];
    
    NSNumberFormatter *numberFormat = [[NSNumberFormatter alloc] init];
    numberFormat.numberStyle = NSNumberFormatterDecimalStyle;
    numberFormat.maximumFractionDigits = 2;
    
    if (pivot.pressure.value) {
        DTEquipmentDataField *pressure = pivot.pressure;
    ...
    if (pivot.flow.value) {
        DTEquipmentDataField *flow = pivot.flow;
    ...
    [self.serviceStopView setValue:pivot.servicePosition units:pivot.servicePositionUom];
    self.serviceStopView.autoRepeat = [pivot.repeatServiceStop boolValue];
    
    if (pivot.chemigation) {
        self.chemigationView.on = [pivot.chemigation.value boolValue];
    } else {
        self.chemigationView.on = NO;
    }
    
    if (pivot.accessoryOne) {
        self.accessoryOneView.on = [pivot.accessoryOne.value boolValue];
    } else {
        self.accessoryOneView.on = NO;
    }
    
    if (pivot.accessoryTwo) {
        self.accessoryTwoView.on = [pivot.accessoryTwo.value boolValue];
    } else {
        self.accessoryTwoView.on = NO;
    }
    
    if (pivot.voltage.value) {
        DTEquipmentDataField *voltage = pivot.voltage;
    ...
    if (pivot.temperature.value) {
        DTEquipmentDataField *temperature = pivot.temperature;
...
- (void)initializeFromConfiguration
{
    ...
    BOOL editable = NO;
    if (pivot.driver && pivot.planId) {
        DTConfiguration *config = [DTConfiguration configurationNamed:pivot.driver inContext:store.managedObjectContext];
        if (config) {
            DTPlan *plan = [DTPlan configuration:config planById:pivot.planId];
            if (plan) {
                editable = YES;
